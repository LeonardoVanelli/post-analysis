...
export async function GET(request: NextRequest) {
  ...
  const posts = await getPosts(maxId, Number(count), accountName);
  ...
  if (post.carousel_media) {
    post.carousel_media.map((cm: any) => {
      const cmFiltered = cm.image_versions2.candidates.find(
        (candidate: any) => candidate.width === 480 && candidate.height === 600
      );
  ...
  return NextResponse.json(postFormatted, { status: 200 });
}
